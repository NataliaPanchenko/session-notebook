...
To join two branches together, you can use `git merge <branchname>`
...
💡 If a fast-forward merge isn’t possible, `git merge` will create a **merge commit** that incorporates the change.

💡 To avoid this and keep a clean history, you can use **`git rebase`**.
...
To join two branches together, you can also use `git rebase`.

In contrast to `git merge`, it **doesn’t create a merge commit**, which results in a **cleaner git history**.
...
When you create a PR and want to merge the feature branch into the main branch, it can happen that somebody made changes to the **same lines of code** you were working on.

Git cannot decide which code is the correct one and marks it as a **conflict**.
...
- Git does not know which commit(s) came first
...
Instead of `git merge`, you can use `git rebase`:
...
Instead of solving a conflict locally, you can use the **GitHub conflict manager** directly in the Pull Request interface.
